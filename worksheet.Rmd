...
* `dv_cli`: a measure of outpatient visits due to Covid-like-illness
...
fcast <- arx_forecaster(
  ...
  # you could try others, see https://parsnip.tidymodels.org
  # while we've tried lots of them, some may fail and some aren't appropriate
  # (e.g. multinom_reg())
  # If you have {ranger} installed, try `rand_forest()`
  # 
  ...
  trainer = linear_reg(),
  arx_args_list(
    # the same for all predictors
    # wrap in a list to differentiate (in the same order)
    lags = c(0, 7, 14),
    ...
    # (other things equal, we'd neet more training data, if we did)
